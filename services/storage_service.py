...
class Article(Base):
    ...
    theme_name = Column(String, nullable=True) # Поле для тега
...
class StorageService:
    ...
    def get_articles_by_status(self, status: Union[str, List[str]], limit: int = 10, random_order: bool = False) -> List[Article]:
        ...
        try:
            ...
            if random_order:
                query = query.order_by(func.random()) # Работает для SQLite
            else:
    ...
    # --- НОВЫЙ МЕТОД ДЛЯ ПОДСЧЕТА СТАТЕЙ ---
    def get_article_count_by_status(self, status: str) -> int:
        """Эффективно считает количество статей с заданным статусом."""
        session = self.Session()
        try:
