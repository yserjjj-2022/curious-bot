...
def run_main_cycle():
    ...
    storage = StorageService(db_url='sqlite:///data/articles.db')
    ...
    for config_path in source_configs:
        try:
            ...
            # --- ИЗМЕНЕНИЯ: Улучшаем логирование ---
            config_name = os.path.basename(config_path)
            source_name_from_config = final_config.get('source_name', 'Unknown')
            
            print(f"\n--- Обрабатываю срез: {config_name} (Источник: {source_name_from_config}) ---")
            # ----------------------------------------
            ...
            for article_meta in articles_to_process:
                ...
                original_abstract = article_meta.get('abstract')
                
                was_added = storage.add_article(
                    article_meta,
                    summary_placeholder,
                    original_abstract,
                    source_name=source_name_from_config
                )
