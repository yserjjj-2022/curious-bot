...
# from agents.investigator_agent import run_investigation_cycle # <-- УДАЛЕНО
...
def run_training_mode():
    ...
    # --- ИЗМЕНЕНО: Последовательность вызовов агентов исправлена ---
    logger.info("--- Этап 1: Сбор статей ---")
    run_collection_cycle(storage, initial_load=False, limit_per_theme=FETCH_LIMIT_TRAINING)
    
    # "Расследование" удалено, так как его логика теперь внутри Экстрактора.
...
def run_production_mode():
    ...
    # --- ИЗМЕНЕНО: Статус 'investigated' удален из проверки ---
    initial_statuses = ['new', 'awaiting_abstract_summary']
    ...
    # scheduler.add_job(run_investigation_cycle, 'interval', minutes=15, args=[StorageService()], name="Investigation_Cycle") # <-- УДАЛЕНО
